...
4. Автоматизирован процесс пересчета целевых метрик, отражающих результаты АБ-тестирования, на основании дополнительных параметров в файле groups_add.csv с оформлением визуализации.
...
7. Выбрал и применил методы статистического анализа для оценки значимости изменений в метриках:
   - **Конверсия из посещений сайта в оплату услуги** (критерий Хи-квадрат Пирсона)
   - **Средний доход с платящего пользователя пользователя или ARPPU** (дисперсии гомогенны, но распределения не подчиняются нормальному закону, более того распределения ассиметричны и имеют выбросы → Bootstrap)
...
10. Реализован SQL-скрипт для подсчета наиболее активных пользователей проекта: наиболее активными пользователеми считаем пользователей, которые решили более 20 заданий на платформе.
